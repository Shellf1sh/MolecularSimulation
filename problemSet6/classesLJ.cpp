...
void VerletSimulation::backintotheBox() {
    for (int i = 0; i < N; ++i) {
        ...
        //std::cout << "New position: ("<<beads[0].x<<", "<<beads[0].y<<", "<<beads[0].z<<")"<<std::endl;
}
...
void VerletSimulation::computeForces() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for(int j = i+1; j < N; j++) {
            ...
            //std::cout << "Before mirror check " << rx;
            ...
            force_magnitude = LennardJones(distance);
    ...
    //std::cout << "Force=(" << beads[0].fx << ", " << beads[0].fy << ", " << beads[0].fz << ")" << std::endl;

}

void VerletSimulation::VerletStep(int step) {
    updateMomenta();
    if(thermo && ((step % 50) == 0) && step < n_steps/2){
        setRandomMomenta();
    }
    updatePositions();
    backintotheBox();
...
double VerletSimulation::getKineticEnergy(){
    ...
    for(int i = 0; i < N; i++){
        ...
        //if(i == 4){std::cout << "Momneta=(" << beads[i].px << ", " << beads[i].py << ", " << beads[i].py << ")" << std::endl;}

        //if(i == 4){std::cout << "Ekin=" << Ekin << " and the momentum in the x direction is " << beads[i].px << std::endl;}
    }
    return Ekin;
...
double VerletSimulation::getPotentialEnergy(){
    ...
    for(int i = 0; i < N; i++){
        for(int j = i+1; j < N; j++){
            ...
            //if(i == 55){std::cout << "Distance=" << distance << std::endl;}
        }
    }
    return Epot;
}
...
double VerletSimulation::getPositionX(int bead){
    return beads[bead].x;
}